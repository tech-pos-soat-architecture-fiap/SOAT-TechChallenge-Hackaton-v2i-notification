...
docker build --no-cache -t hackaton-v2i-notification:v4 .

# Copiar imagem para o minikube
echo "📦 Copiando imagem para o minikube..."
minikube image load hackaton-v2i-notification:v4
...
kubectl rollout status deployment/v2i-notification-deployment-v1 --timeout=180s
